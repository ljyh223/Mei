...
class HomeRepository(private val weApiService: WeApiService) {
    ...
    suspend fun getHomePageResourceShow(
        ...
        if (isNewDay(getLastFetchTime(context)) || refresh) {
            ...
            val page1 = weApiService.getHomePageResourceShow(GetHomePageResourceShow(refresh= refresh.toString()))
            val page2 = weApiService.getHomePageResourceShow(GetHomePageResourceShow(cursor = 1, refresh= refresh.toString()))
    ...
    private suspend fun getLastHomePage(
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (file.exists())  {
                ...
                Gson().fromJson(json, object : TypeToken<List<HomePageResourceShow.Data.Block>>() {}.type)
            } else {
                emptyList()
            }
        }
